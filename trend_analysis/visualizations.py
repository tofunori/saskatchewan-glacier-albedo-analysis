"""
Visualisations pour l'analyse de tendances d'albédo
==================================================

Ce module contient tous les graphiques et visualisations pour l'analyse
des tendances d'albédo du glacier Saskatchewan.
"""
...
class AlbedoVisualizer:
    """
    Classe pour créer toutes les visualisations d'albédo
    """
    ...
    def create_trend_overview_graph(self, trend_results, variable='mean', save_path=None):
        ...
        fig.suptitle(f'Tendances d\'Albédo par Fraction - {variable.title()}', 
        ...
        for idx, fraction in enumerate(self.fraction_classes):
            ...
            if not np.isnan(sen_slope):
                ...
                ax.plot(times, trend_line, color='red', linewidth=2, alpha=0.8,
                       label=f'Pente Sen: {sen_slope*10:.6f}/décennie')
        ...
        if save_path is None:
            save_path = f'trend_overview_{variable}.png'
    ...
    def create_time_series_graph(self, fraction, variable='mean', save_path=None):
        ...
        try:
            fraction_data = self.data_loader.get_fraction_data(fraction, variable, dropna=True)
        ...
        if save_path is None:
            save_path = f'timeseries_{fraction}_{variable}.png'
    ...
    def create_correlation_matrix_graph(self, variable='mean', save_path=None):
        ...
        if save_path is None:
            save_path = f'correlation_matrix_{variable}.png'
    ...
    def create_seasonal_patterns_graph(self, variable='mean', save_path=None):
        ...
        if save_path is None:
            save_path = f'seasonal_patterns_{variable}.png'
    ...
    def create_summary_dashboard(self, basic_results, variable='mean', save_path=None):
        """
        Crée un dashboard de résumé avec tous les graphiques principaux
        ...
        if save_path is None:
            save_path = f'dashboard_summary_{variable}.png'
        ...
        for fraction, result in basic_results.items():
            if not result.get('error', False):
                ...
                if not np.isnan(slope):
                    fractions.append(self.class_labels[fraction][:10])  # Raccourcir les labels
                    slopes.append(slope)
                    colors.append(FRACTION_COLORS.get(fraction, 'gray'))
        
        if fractions:
            bars = ax.bar(fractions, slopes, color=colors, alpha=0.7)
