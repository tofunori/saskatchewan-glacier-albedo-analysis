// ╔════════════════════════════════════════════════════════════════════════════════════════╗
// ║        ANALYSE ALBÉDO GÉNÉRAL PAR FRACTION ET ÉLÉVATION - SASKATCHEWAN GLACIER        ║
// ║                         MODIS MCD43A3.061 + SRTM DEM 2010-2024                        ║
// ║                    Méthodologie Williamson & Menounos (2021) + Fractions              ║
// ╚════════════════════════════════════════════════════════════════════════════════════════╝

// Description : Analyse l'évolution de l'albédo général selon les fractions de couverture 
...
function analyzeDailyGeneralAlbedoByFractionAndElevation(img) {
  var date = img.date();
  var quality = img.select('BRDF_Albedo_Band_Mandatory_Quality_shortwave');
  var general_albedo = img.select('Albedo_WSA_shortwave').multiply(0.001);
  ...
  // Masques de qualité pour MCD43A3: 0=Best, 1=Good
  ...
  keyClassNames.forEach(function(className) {
    elevationNames.forEach(function(elevName) {
      ...
      var validGeneralAlbedo = general_albedo.updateMask(combinedMask);
      
      // Calculer statistiques complètes pour cette combinaison fraction × élévation
      var classStats = validGeneralAlbedo.reduceRegion({
      ...
      stats[prefix + '_mean'] = classStats.get('Albedo_WSA_shortwave_mean');
      stats[prefix + '_median'] = classStats.get('Albedo_WSA_shortwave_median');
      stats[prefix + '_count'] = pixelCount;
      stats[prefix + '_stddev'] = classStats.get('Albedo_WSA_shortwave_stdDev');
      stats[prefix + '_min'] = classStats.get('Albedo_WSA_shortwave_min');
      stats[prefix + '_max'] = classStats.get('Albedo_WSA_shortwave_max');
...
print('📅 Traitement des données MCD43A3 2010-2024 (juin-septembre)...');
...
// Charger la collection complète MCD43A3 pour l'analyse quotidienne
var dailyGeneralCollection = ee.ImageCollection('MODIS/061/MCD43A3')
...
var dailyGeneralAlbedoByFractionElevation = dailyGeneralCollection
  .map(analyzeDailyGeneralAlbedoByFractionAndElevation);

print('✅ Statistiques quotidiennes fraction × élévation calculées:', 
      dailyGeneralAlbedoByFractionElevation.size(), 'composites 16-jours');

// Validation rapide de la structure
var firstFeature = ee.Feature(dailyGeneralAlbedoByFractionElevation.first());
...
var calculateElevationTrends = function(className, elevName) {
  ...
  var validData = dailyGeneralAlbedoByFractionElevation
...
var updateVisualization = function() {
  ...
  var selectedImage = ee.ImageCollection('MODIS/061/MCD43A3')
    .filterDate(selectedDate, selectedDate.advance(16, 'day'))
    .filterBounds(glacier_geometry)
    .select(['Albedo_WSA_shortwave', 'BRDF_Albedo_Band_Mandatory_Quality_shortwave'])
    .first();
  ...
  // Préparer albédo général
  var selectedGeneralAlbedo = selectedImage.select('Albedo_WSA_shortwave')
    .multiply(0.001)
    .updateMask(selectedImage.select('BRDF_Albedo_Band_Mandatory_Quality_shortwave').lte(1))
    .updateMask(glacier_mask);
  
  // Paramètres de visualisation
  var generalAlbedoVis = {min: 0.3, max: 0.9, palette: ['darkblue', 'blue', 'cyan', 'yellow', 'orange', 'red']};
  ...
  var layersToUpdate = [
    ...
    {
      image: selectedGeneralAlbedo.updateMask(selectedFractionMasks.mostly_ice),
      vis: generalAlbedoVis,
      name: '7. Albédo mostly_ice (75-90%)',
      visible: false
    },
    {
      image: selectedGeneralAlbedo.updateMask(selectedFractionMasks.pure_ice),
      vis: generalAlbedoVis,
      name: '8. Albédo pure_ice (90-100%)',
      visible: true
    },
    {
      image: selectedGeneralAlbedo.updateMask(selectedFractionMasks.mostly_ice.and(selectedElevationZones.above_median)),
      vis: generalAlbedoVis,
      name: '9. Mostly_ice × Above_median',
      visible: false
    },
    {
      image: selectedGeneralAlbedo.updateMask(selectedFractionMasks.mostly_ice.and(selectedElevationZones.at_median)),
      vis: generalAlbedoVis,
      name: '10. Mostly_ice × At_median',
      visible: false
    },
    {
      image: selectedGeneralAlbedo.updateMask(selectedFractionMasks.mostly_ice.and(selectedElevationZones.below_median)),
      vis: generalAlbedoVis,
      name: '11. Mostly_ice × Below_median',
      visible: false
    },
    {
      image: selectedGeneralAlbedo.updateMask(selectedFractionMasks.pure_ice.and(selectedElevationZones.above_median)),
      vis: generalAlbedoVis,
      name: '12. Pure_ice × Above_median',
      visible: true
    },
    {
      image: selectedGeneralAlbedo.updateMask(selectedFractionMasks.pure_ice.and(selectedElevationZones.at_median)),
      vis: generalAlbedoVis,
      name: '13. Pure_ice × At_median',
      visible: true
    },
    {
      image: selectedGeneralAlbedo.updateMask(selectedFractionMasks.pure_ice.and(selectedElevationZones.below_median)),
      vis: generalAlbedoVis,
...
var example_image = ee.ImageCollection('MODIS/061/MCD43A3')
  .filterDate(example_date, example_date.advance(16, 'day'))
  .filterBounds(glacier_geometry)
  .select(['Albedo_WSA_shortwave', 'BRDF_Albedo_Band_Mandatory_Quality_shortwave'])
  .first();
...
var example_general_albedo = example_image.select('Albedo_WSA_shortwave')
  .multiply(0.001)
...
var combinationExamples = example_general_albedo.rename('general_albedo').float()
  .addBands(example_general_albedo.updateMask(exampleFractionMasks.mostly_ice).rename('mostly_ice_albedo').float())
  .addBands(example_general_albedo.updateMask(exampleFractionMasks.pure_ice).rename('pure_ice_albedo').float())
  .addBands(example_general_albedo.updateMask(exampleFractionMasks.mostly_ice.and(example_elevation_zones.above_median)).rename('mostly_ice_above_median').float())
  .addBands(example_general_albedo.updateMask(exampleFractionMasks.mostly_ice.and(example_elevation_zones.at_median)).rename('mostly_ice_at_median').float())
  .addBands(example_general_albedo.updateMask(exampleFractionMasks.mostly_ice.and(example_elevation_zones.below_median)).rename('mostly_ice_below_median').float())
  .addBands(example_general_albedo.updateMask(exampleFractionMasks.pure_ice.and(example_elevation_zones.above_median)).rename('pure_ice_above_median').float())
  .addBands(example_general_albedo.updateMask(exampleFractionMasks.pure_ice.and(example_elevation_zones.at_median)).rename('pure_ice_at_median').float())
  .addBands(example_general_albedo.updateMask(exampleFractionMasks.pure_ice.and(example_elevation_zones.below_median)).rename('pure_ice_below_median').float());
...
var analysisMetadata = ee.FeatureCollection([
  ee.Feature(null, {
    'analysis_name': 'Saskatchewan_Glacier_General_Albedo_Fraction_Elevation_Analysis',
    ...
    'modis_product': 'MCD43A3.061_Albedo_WSA_shortwave',
    'dem_source': 'SRTM_500m_resolution',
    'quality_thresholds': 'MCD43A3_QA_0-1',
    ...
    'notes': 'Complete_elevation-dependent_analysis_combining_glacier_fractions_with_Williamson_Menounos_elevation_zones_general_albedo'
  })
]);
...
print('• daily_general_albedo_fraction_elevation_williamson_method_2010_2024.csv');
print('  → Données composites 16-jours complètes (1830 observations × 6 combinaisons)');
print('• general_albedo_elevation_trends_williamson_menounos_method_2010_2024.csv');
print('  → Analyses de tendance Mann-Kendall par combinaison');
print('• dem_elevation_zones_general_albedo_williamson_method_500m.tif');
print('  → Carte DEM + zones élévation + masque glacier');
print('• general_albedo_fraction_elevation_combinations_example_20200715.tif');
print('  → Exemple albédo par combinaison (validation visuelle)');
print('• general_albedo_analysis_metadata_williamson_menounos_method.csv');
...
print('✅ Données: Composites 16-jours × 6 combinaisons fraction × élévation');
...
print('• Comparer avec données MOD10A1 (Snow Albedo)');
print('• Analyser différences temporelles: Daily vs 16-day composite');
print('');
