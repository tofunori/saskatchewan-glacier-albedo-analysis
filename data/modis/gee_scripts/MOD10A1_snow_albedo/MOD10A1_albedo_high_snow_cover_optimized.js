...
// 4. Fonction optimisée pour créer classification de fraction (au lieu de 5 masques)
function createFractionClassification(fractionImage, thresholds) {
  return ee.Image.expression(
    '(fraction > 0) * (' +
    '(fraction < t1) * 1 + ' +
    '(fraction >= t1 && fraction < t2) * 2 + ' +
    '(fraction >= t2 && fraction < t3) * 3 + ' +
    '(fraction >= t3 && fraction < t4) * 4 + ' +
    '(fraction >= t4) * 5)',
    {
      'fraction': fractionImage,
      't1': thresholds[0],
      't2': thresholds[1], 
      't3': thresholds[2],
      't4': thresholds[3]
    }
  ).rename('fraction_class');
}
...
function calculateAnnualAlbedoHighSnowCoverOptimized(year) {
  ...
  var processed_collection = mod10a1_collection.map(function(img) {
    ...
    // Classification de fraction au lieu de 5 masques
    var fraction_classes = createFractionClassification(STATIC_GLACIER_FRACTION, FRACTION_THRESHOLDS);
    
    // Masquer par fraction classes et albédo valide
    var masked_data = albedo_scaled.addBands(fraction_classes)
      .updateMask(combined_mask);
  ...
  // Statistiques par groupe avec reduceRegion optimisé
...
function analyzeDailyAlbedoHighSnowCoverOptimized(img) {
  ...
  // Classification de fraction
  var fraction_classes = createFractionClassification(STATIC_GLACIER_FRACTION, FRACTION_THRESHOLDS);
  
  // Statistiques groupées par classe
  var classStats = albedo_scaled.addBands(fraction_classes)
    .reduceRegion({
      reducer: ee.Reducer.mean().combine(
  ...
  var total_filtered = combined_mask.updateMask(STATIC_GLACIER_FRACTION.gt(0)).reduceRegion({
    ...
    maxPixels: 1e9,
    tileScale: 4
  }).get('NDSI_Snow_Cover_Basic_QA'); // Nom cohérent après masquage
...
var updateFiltering = function() {
  ...
  albedoRange.evaluate(function(range) {
    var minVal = range['Snow_Albedo_Daily_Tile_min'] || 0.4;
    var maxVal = range['Snow_Albedo_Daily_Tile_max'] || 0.9;
  ...
  dayStats.evaluate(function(stats) {
    ...
    var pixelCount = stats['Snow_Albedo_Daily_Tile_count'] || 0;
